#ifndef RECOMPUTREGIONS_HPP
#define RECOMPUTREGIONS_HPP
...
class ReComputeRegions {
    public:
        ...
        * Constructor
        */
        ReComputeRegions(cv::Mat img, float _pourcentByRep, unsigned int _rep = 16);
        ...
        */
        std::queue<RegionRe *> calculateRegions(std::queue<RegionRe *> & regToCal);
        ...
        */
        void calculatQueueRegion(std::queue<RegionRe *> reg, int nbrCallMax = 100);
        ...
        */
        void updateBorder (RegionRe * r);

        /**
        * Merge a Region 
        */
        RegionRe * mergeRegion(const int id, std::unordered_set<int> & alereadyMerge, std::unordered_set<int> & mergeInidice, int iteration, int & regTraited);
